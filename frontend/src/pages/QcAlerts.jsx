...
const QcAlerts = () => {
  ...
  const pollBadge = useCallback(async () => {
    try {
      const res = await axios.get('/api/qc/alerts/unread/count', { headers: auth() });
      const count = res.data?.data?.count ?? 0;
      if (count > prevUnread.current) {
        // New alerts arrived — do a silent full reload
        load(true);
      }
  ...
  useEffect(() => {
    ...
    // Silent reload every 4 hours — just refreshes the list, does NOT create alerts
    const resetOverdue = () => {
      setNextRefresh(Date.now() + OVERDUE_POLL_MS);
      overdueTimer.current = setTimeout(() => {
        load(false); // silent=false → no "refreshed" toast, just quiet reload
        resetOverdue();
      }, OVERDUE_POLL_MS);
    };
    resetOverdue();
  ...
  useEffect(() => {
    countdownTimer.current = setInterval(() => {
      setNextRefresh(prev => prev); // trigger re-render for countdown
    }, 60_000);
  ...
  const handleMarkRead = async (alertId) => {
    ...
    catch { toast.error('Failed'); load(true); }
  };
  ...
  const doDelete = async (ids) => {
    try {
      await Promise.all(ids.map(id =>
        axios.delete(`/api/qc/alerts/${id}`, { headers: auth() })
      ));
  ...
  const selectAll = () => setSelected(new Set(filtered.map(a => a.alertId)));
    ...
    <div className="qa-page">
      ...
      <div className="qa-header">
        ...
        <div className="qa-header-actions">
          ...
          <button className="qa-btn-secondary" onClick={() => load(true)} disabled={refreshing}>
      ...
      <div className="qa-mgmt-row">
        <button className="qa-mgmt-btn" onClick={selectAll}>
