...
public class BoxController {
    ...
    @PostMapping
    @Operation(summary = "Create box", description = "Create a new box (box number auto-generated when omitted)")
    public ResponseEntity<ApiResponse<Box>> createBox(
            @RequestBody BoxRequest req,
            Authentication authentication) {
        // boxNumber may be null/blank — BoxService auto-generates B<n> per rack
        log.info("POST /api/boxes | rackId={}, number='{}', label='{}'", req.rackId(), req.boxNumber(), req.boxLabel());
        ...
        log.info("Box created, id={}, number={}", box.getBoxId(), box.getBoxNumber());
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.success("Box created successfully", box));
    }

    @PutMapping("/{id}")
    @Operation(summary = "Update box", description = "Update an existing box (blank box number keeps existing)")
    public ResponseEntity<ApiResponse<Box>> updateBox(
            @PathVariable Long id,
            @RequestBody BoxRequest req) {
        log.info("PUT /api/boxes/{} | number='{}'", id, req.boxNumber());
